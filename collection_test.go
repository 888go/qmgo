...
func TestCollection_EnsureIndex(t *testing.T) {
	...
	cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"id1"}, IndexOptions: indexOpts}})
	cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"id2", "id3"}}})
	cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"id4", "-id5"}}})

	// same index，error
	ast.Error(cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"id1"}}}))
...
func TestCollection_CreateIndexes(t *testing.T) {
	...
	ast.NoError(cli.X创建索引(context.Background(), options.IndexModel{X索引字段: unique, IndexOptions: indexOpts}))

	ast.NoError(cli.X创建多条索引(context.Background(), []options.IndexModel{{X索引字段: []string{"id2", "id3"}},
		{X索引字段: []string{"id4", "-id5"}}}))
	// same index，error
	ast.Error(cli.X创建索引(context.Background(), options.IndexModel{X索引字段: unique}))
...
func TestCollection_DropIndex(t *testing.T) {
	...
	indexOpts := officialOpts.Index()
	indexOpts.SetUnique(true)
	cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"index1"}, IndexOptions: indexOpts}})

	// same index，error
	ast.Error(cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"index1"}}}))

	err := cli.X删除索引(context.Background(), []string{"index1"})
	ast.NoError(err)
	ast.NoError(cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"index1"}}}))
	...
	indexOpts = officialOpts.Index()
	indexOpts.SetUnique(true)
	cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"index2", "-index1"}, IndexOptions: indexOpts}})
	ast.Error(cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"index2", "-index1"}}}))
	err = cli.X删除索引(context.Background(), []string{"index2", "-index1"})
	ast.NoError(err)
	ast.NoError(cli.ensureIndex(context.Background(), []options.IndexModel{{X索引字段: []string{"index2", "-index1"}}}))
