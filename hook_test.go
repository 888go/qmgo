...
func TestUpdateHook(t *testing.T) {
	...
	err = cli.X更新一条(ctx, bson.M{"name": "Lucas"}, bson.M{操作符.X设置值: bson.M{"age": 27}}, options.UpdateOptions{
		UpdateHook: uh,
	})
	ast.NoError(err)
	...
	err = cli.X更新并按ID(ctx, res.InsertedID, bson.M{操作符.X设置值: bson.M{"age": 27}}, options.UpdateOptions{
		UpdateHook: uh,
	})
	ast.NoError(err)
	...
	cli.X更新(ctx, bson.M{"name": "Lucas"}, bson.M{操作符.X设置值: bson.M{"age": 27}}, options.UpdateOptions{
		UpdateHook: uh,
	})
	ast.NoError(err)
...
func TestHookErr(t *testing.T) {
	...
	_, err = cli.X更新(ctx, bson.M{"name": "Lucas"}, bson.M{操作符.X设置值: bson.M{"age": 27}}, options.UpdateOptions{
