...
// X文档集合 是一个MongoDB集合的句柄 md5:be1b94030609bdd1
type X文档集合 struct {
...
// X查询 通过条件过滤并查找，返回QueryI md5:bda4cc0c85d800a1
...
// X插入多个 执行插入命令，将多个文档插入到集合中。如果opts中的InsertHook被设置，将在其上应用钩子；否则，尝试将doc作为钩子使用。
// 参考：https://docs.mongodb.com/manual/reference/command/insert/
...
// X替换插入 如果过滤器匹配，则更新一个文档，如果过滤器不匹配，则插入一个文档。如果过滤器无效，会返回错误。
...
// X替换插入并按ID 如果id匹配，则更新一个文档，如果id不匹配，则插入一个新的文档，并将id注入到文档中。
...
// X替换一条 执行更新命令，最多更新集合中的一个文档。如果 opts 中的 UpdateHook 被设置，那么 Hook 将在其上执行，否则 Hook 尝试将 doc 作为 Hook。预期 doc 的类型是用户定义的文档的定义。
...
// X删除并按ID 执行删除命令，从集合中删除最多一个文档。 md5:6516d8a8963d018c
...
// X删除 执行一个删除命令，从集合中删除文档。如果 filter 是 bson.M{}（空映射），则会删除集合中的所有文档。
// 参考：https://docs.mongodb.com/manual/reference/command/delete/
...
// X聚合 在集合上执行聚合命令，并返回一个 AggregateI，用于获取结果文档。 md5:e57ffed517c59fbc
...
// EnsureIndexes弃用 在集合中创建唯一和非唯一的索引，与CreateIndexes的组合不同：
...
// X删除全部索引 会删除集合上除了_id字段索引之外的所有索引
...
// X删除索引 从集合中删除索引，需要删除的索引应与输入的索引列表匹配
...
// X取副本 创建集合的副本 md5:5df787f1c8ebab26
func (c *X文档集合) X取副本() (*mongo.Collection, error) {
	return c.collection.Clone()
}

// X取集合名 返回集合的名字 md5:440484db8f2a466d
func (c *X文档集合) X取集合名() string {
	return c.collection.Name()
}

// X取变更流 返回对应集合上所有更改的流。有关更改流的更多信息，请参阅
