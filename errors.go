...
var (
	// X错误_结果参数_必须切片指针 如果结果参数不是一个切片的指针，返回此错误 md5:99bf2bfe686c166d
	X错误_结果参数_必须切片指针 = errors.New("result argument must be a pointer to a slice")
	// X错误_结果参数_必须切片地址 如果结果参数不是切片的地址时返回错误 md5:a70365d8a017acd7
	X错误_结果参数_必须切片地址 = errors.New("result argument must be a slice address")
	// X错误_查询结果_类型不一致 如果查询结果类型与MongoDB值类型不一致时返回 md5:940d09b0f6052a9f
	X错误_查询结果_类型不一致 = errors.New("result type is not equal mongodb value type")
	// X错误_结果值不能更改 如果结果值不能更改，返回这个错误 md5:95f73ebb72c4463a
	X错误_结果值不能更改 = errors.New("the value of result can not be changed")
	// X错误_未找到文档 如果未找到文档，则返回此错误 md5:381e7dce5c56bc42
	X错误_未找到文档 = mongo.ErrNoDocuments
	// X错误_事务_重试 如果事务需要重试时返回该错误 md5:82e274f71f7c0175
	X错误_事务_重试 = errors.New("retry transaction")
	// X错误_事务_不支持 如果不支持事务，则返回该错误 md5:97ad2e1fbe2e7207
	X错误_事务_不支持 = errors.New("transaction not supported")
	// X错误_不支持用户名 如果用户名无效，则返回此错误 md5:c9df5d462362cad6
	X错误_不支持用户名 = errors.New("username not supported")
	// X错误_不支持密码 如果密码无效时返回 md5:e9df4f7f8304cc70
	X错误_不支持密码 = errors.New("password not supported")
	// X错误_插入_无效切片 如果插入的参数不是一个有效的切片，返回该错误 md5:2b940e5853972183
	X错误_插入_无效切片 = errors.New("must be valid slice to insert")
	// X错误_替换_文档含更新操作符 如果替换文档中包含更新操作符，返回错误 md5:4f127578930f91fa
	X错误_替换_文档含更新操作符 = errors.New("replacement document cannot contain keys beginning with '$'")
)

// X是否为无文档错误 检查错误是否表示没有找到文档，既包括 mongo-go-driver 的错误也包括 qmgo 自定义的错误
...
// X是否为重复键错误 检查错误是否为MongoDB的E11000（重复错误）。 md5:4a3435e9efa67970
