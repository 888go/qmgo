...
func TestFieldInsert(t *testing.T) {
	...
	// default fields
	ast.NotEqual(time.Time{}, ur.X创建时间)
	ast.NotEqual(time.Time{}, ur.X更新时间)
...
func TestFieldInsertMany(t *testing.T) {
	...
	// default fields
	ast.NotEqual(time.Time{}, ur[0].X创建时间)
	ast.NotEqual(time.Time{}, ur[0].X更新时间)
	ast.NotEqual(primitive.NilObjectID, ur[0].Id)
	// default fields
	ast.NotEqual(time.Time{}, ur[1].X创建时间)
	ast.NotEqual(time.Time{}, ur[1].X更新时间)
...
func TestFieldUpdate(t *testing.T) {
	...
	ast.Equal(time.Time{}, findUi.X更新时间)
	...
	ast.NotEqual(time.Time{}, findUi.X更新时间)
...
func TestFieldUpsert(t *testing.T) {
	...
	result, err := cli.X替换插入(ctx, bson.M{"_id": id}, u)
	ast.NoError(err)
	fmt.Println(result)
	...
	fmt.Println(tBefore3s.Unix(), ui.X创建时间.Unix())
	ast.Equal(tBefore3s.Unix(), ui.X创建时间.Unix())
	ast.Equal(tBefore3s.Unix(), ui.CreateTimeAt.Unix())
	ast.NotEqual(tBefore3s.Unix(), ui.X更新时间.Unix())
...
func TestFieldUpdateId(t *testing.T) {
	...
	err = cli.X更新并按ID(context.Background(), res.X插入ID, bson.M{"$set": bson.M{"updateTimeAt": 0, "updateAt": time.Time{}}})
	ast.NoError(err)
